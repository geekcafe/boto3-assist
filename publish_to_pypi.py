                ...
                if pkg_dir.is_dir():
                    ...
                    if not version_file.exists():
                        with open(version_file, "w") as f:
                            f.write(
                                f'''"""Version information for {pkg_name}."""
...
def build_package():
    ...
    try:
        ...
        if "401 Error" in output and "codeartifact" in output.lower():
            ...
            print_info(
                "This error occurs when your AWS CodeArtifact credentials have expired or are invalid."
            )
            print_info(
                "The global pip configuration is trying to use AWS CodeArtifact instead of PyPI."
            )
            ...
            if found_configs:
                fix_config = (
                    input(
                        "\nWould you like to temporarily rename these pip config files for this build? (y/N): "
                    )
                    .strip()
                    .lower()
                )
                if fix_config == "y":
                    ...
                    try:
                        ...
                        restore = (
                            input(
                                "\nWould you like to restore the original pip config files? (Y/n): "
                            )
                            .strip()
                            .lower()
                        )
                ...
                print_info(
                    "You might want to check for environment variables like AWS_PROFILE or custom pip configurations."
                )

            print_info("\nAlternative solutions:")
            print_info(
                "1. Create a local virtual environment with 'python -m venv .venv' and activate it"
            )
            print_info("2. Use '--no-build-isolation' flag with pip if you're installing locally")
            print_info(
                "3. Temporarily set PIP_INDEX_URL environment variable: export PIP_INDEX_URL=https://pypi.org/simple/"
            )

            # Offer to create a local pip.conf file
            create_local = (
                input(
                    "\nWould you like to create a local pip.conf file that points to PyPI? (y/N): "
                )
                .strip()
                .lower()
            )
...
"""
                    ...
                    print_info(
                        "This configuration will take precedence over global settings when running pip in this directory."
                    )
...
"""
    ...
    print_info(
        "You can generate tokens at https://pypi.org/manage/account/ and https://test.pypi.org/manage/account/"
    )
...
def check_for_script_updates():
    ...
    if "--ci" in sys.argv or os.environ.get("PYPI_SKIP_UPDATE_CHECK", "").lower() in (
        "1",
        "true",
        "yes",
    ):
...
def main():
    ...
    if not pypirc_path:
        try:
            subprocess.run(
                [sys.executable, "-m", "twine", "check", "--strict", "README.md"],
                check=True,
                capture_output=True,
            )
        except subprocess.CalledProcessError:
