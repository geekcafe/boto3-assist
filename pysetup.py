...
class ProjectSetup:
    ...
    def _login_to_codeartifact(self, ca_repo):
        ...
        try:
            ...
            result = subprocess.run(cmd + ["--dry-run"], capture_output=True, text=True, env=env)
    ...
    def _maybe_setup_nexus(self, force_prompt=False) -> bool:
        ...
        if nexus_repo and not force_prompt:
            ...
            if ans != "y":
                ...
                if "repositories" not in self.ca_settings:
                    self.ca_settings["repositories"] = {}
                self.ca_settings["repositories"]["nexus"] = {"type": "nexus", "enabled": False}
                # Save the updated settings
                self.CA_CONFIG.write_text(json.dumps(self.ca_settings, indent=2))
                print_info("Nexus disabled and saved to configuration.")
                return False
    ...
    def _maybe_setup_github_packages(self, force_prompt=False) -> bool:
        ...
        if gh_repo and not force_prompt:
            ...
            if ans != "y":
                ...
                if "repositories" not in self.ca_settings:
                    self.ca_settings["repositories"] = {}
                self.ca_settings["repositories"]["github"] = {"type": "github", "enabled": False}
                # Save the updated settings
                self.CA_CONFIG.write_text(json.dumps(self.ca_settings, indent=2))
                print_info("GitHub Packages disabled and saved to configuration.")
                return False
    ...
    def _maybe_setup_azure_artifacts(self, force_prompt=False) -> bool:
        ...
        if azure_repo and not force_prompt:
            ...
            if ans != "y":
                ...
                if "repositories" not in self.ca_settings:
                    self.ca_settings["repositories"] = {}
                self.ca_settings["repositories"]["azure"] = {"type": "azure", "enabled": False}
                # Save the updated settings
                self.CA_CONFIG.write_text(json.dumps(self.ca_settings, indent=2))
                print_info("Azure Artifacts disabled and saved to configuration.")
                return False
            ...
            azure_repo = {
                ...
                "username": input("   Username (typically just use any string): ").strip()
                or "azure",
    ...
    def _maybe_setup_google_artifact_registry(self, force_prompt=False) -> bool:
        ...
        if gcp_repo and not force_prompt:
            ...
            reuse = (
                input("Do you want to use Google Artifact Registry? (Y/n): ").strip().lower() or "y"
            )
            if reuse != "y":
                # Disable the repository but keep the settings
            ...
            if ans != "y":
                ...
                if "repositories" not in self.ca_settings:
                    self.ca_settings["repositories"] = {}
                self.ca_settings["repositories"]["google"] = {"type": "google", "enabled": False}
                # Save the updated settings
                self.CA_CONFIG.write_text(json.dumps(self.ca_settings, indent=2))
                print_info("Google Artifact Registry disabled and saved to configuration.")
                return False
            ...
            while True:
                ...
                if output:
                    ...
                    if "Collecting" in line:
                        ...
                        if packages_match:
                            packages_installed = [
                                pkg.strip() for pkg in packages_match.group(1).split(",")
                            ]
                        ...
                        if success_match:
                            packages_installed = [
                                pkg.split("-")[0] for pkg in success_match.group(1).split()
                            ]
                ...
                try:
                    with open(pyproject_path, "r", encoding="utf-8") as f:
                        ...
                        poetry_match = re.search(
                            r'\[tool\.poetry\][^\[]*description\s*=\s*"([^"]*)"', content
                        )
                        if poetry_match:
...
"""
    ...
    def _check_git_setup(self):
        ...
        if not git_dir.exists():
            ...
            response = (
                input("Would you like to initialize a Git repository? (Y/n): ").strip().lower()
                or "y"
            )
            ...
            if response.startswith("y"):
                try:
                    ...
                    commit_response = (
                        input("Would you like to make the initial commit? (Y/n): ").strip().lower()
                        or "y"
                    )
                    ...
                    if commit_response.startswith("y"):
                        ...
                        commit_message = (
                            input("Enter commit message (default: 'Initial commit'): ").strip()
                            or "Initial commit"
                        )
...
"""
        ...
        # Otherwise, check if user has already been prompted about adding .pysetup.json to gitignore
            ...
            response = (
                input("Would you like to exclude .pysetup.json from git tracking? (Y/n): ")
                .strip()
                .lower()
                or "y"
            )
...
"""
    ...
    def _get_setup_sh_update_preference(self, force_prompt=False) -> str:
        ...
        if hasattr(self, "_ci_mode") and self._ci_mode:
            ...
            while True:
                response = (
                    input("pysetup.sh update preference [interactive/yes/no]: ").strip().lower()
                )
    ...
    def _get_repo_update_preference(self, force_prompt=False) -> str:
        ...
        if hasattr(self, "_ci_mode") and self._ci_mode:
            ...
            while True:
                response = (
                    input("Repository update preference [interactive/yes/no]: ").strip().lower()
                )
                if response in ("", "interactive"):
                    update_preference = "interactive"
                    break
    ...
    def _get_env_action_preference(self, force_prompt=False) -> str:
        ...
        while True:
            response = (
                input("Environment action preference [reuse/clean/upgrade]: ").strip().lower()
            )
    ...
    def _setup_pip(self):
        ...
        try:
            ...
            self._run_pip_command_with_progress(["install", "--upgrade", "pip"], "Upgrading pip")
        ...
        try:
            ...
            if which("poetry") is not None:
                ...
                if shell in ("bash", "zsh"):
                    ...
                    self.__exit_notes.append("🔄 Then reload your shell (e.g. exec $SHELL -l).")
