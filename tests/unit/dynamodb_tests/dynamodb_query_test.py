...
class DbQueryTest(unittest.TestCase):
    ...
    def test_query_with_pagination_start_key(self):
        ...
        user = User()
        key = user.get_key("gsi1")
        response = self.db.query(table_name=self.__table_name, key=key, limit=5)
        ...
        response2 = self.db.query(
            table_name=self.__table_name, key=key, limit=5, start_key=last_key
        )
        ...
        self.assertIn(
            "LastEvaluatedKey",
            response2,
            "Should still have LastEvaluatedKey since more items exist",
        )
        ...
        self.assertEqual(
            len(first_batch_ids.intersection(second_batch_ids)),
            0,
        ...
        while next_start_key:
            response_next = self.db.query(
                table_name=self.__table_name, key=key, limit=5, start_key=next_start_key
            )
