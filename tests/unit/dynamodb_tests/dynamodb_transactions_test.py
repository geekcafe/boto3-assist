...
class TestTransactWriteItems(unittest.TestCase):
    ...
    def test_transact_write_put_operations(self):
        ...
        user1 = self.db.get(
            key={"pk": "user#user-001", "sk": "user#user-001"}, table_name=self.table_name
        )
        user2 = self.db.get(
            key={"pk": "user#user-002", "sk": "user#user-002"}, table_name=self.table_name
        )
    ...
    def test_transact_write_update_operations(self):
        ...
        account1 = self.db.get(
            key={"pk": "account#123", "sk": "account#123"}, table_name=self.table_name
        )
        account2 = self.db.get(
            key={"pk": "account#456", "sk": "account#456"}, table_name=self.table_name
        )
    ...
    def test_transact_write_delete_operations(self):
        ...
        operations = [
            {"Delete": {"TableName": self.table_name, "Key": {"pk": "temp#001", "sk": "temp#001"}}},
            {"Delete": {"TableName": self.table_name, "Key": {"pk": "temp#002", "sk": "temp#002"}}},
        ]
        ...
        result1 = self.db.get(key={"pk": "temp#001", "sk": "temp#001"}, table_name=self.table_name)
        result2 = self.db.get(key={"pk": "temp#002", "sk": "temp#002"}, table_name=self.table_name)
    ...
    def test_transact_write_mixed_operations(self):
        ...
        new_user = self.db.get(key={"pk": "user#new", "sk": "user#new"}, table_name=self.table_name)
        self.assertIn("Item", new_user)
        self.assertEqual(new_user["Item"]["name"], "NewUser")

        # Verify existing item updated
        existing = self.db.get(
            key={"pk": "user#existing", "sk": "user#existing"}, table_name=self.table_name
        )
    ...
    def test_transact_write_with_condition(self):
        ...
        account = self.db.get(
            key={"pk": "account#789", "sk": "account#789"}, table_name=self.table_name
        )
    ...
    def test_transact_write_too_many_operations(self):
        ...
        operations = [
            {"Put": {"TableName": self.table_name, "Item": {"pk": f"user#{i}", "sk": f"user#{i}"}}}
            for i in range(101)
        ]
...
class TestTransactGetItems(unittest.TestCase):
    ...
    def test_transact_get_basic(self):
        ...
        keys = [
            {"Key": {"pk": "user#user-0", "sk": "user#user-0"}, "TableName": self.table_name},
            {"Key": {"pk": "user#user-1", "sk": "user#user-1"}, "TableName": self.table_name},
        ]
    ...
    def test_transact_get_multiple_items(self):
        ...
        keys = [
            {"Key": {"pk": f"user#user-{i}", "sk": f"user#user-{i}"}, "TableName": self.table_name}
            for i in range(5)
        ]
    ...
    def test_transact_get_decimal_conversion(self):
        ...
        keys = [{"Key": {"pk": "user#user-0", "sk": "user#user-0"}, "TableName": self.table_name}]
    ...
    def test_transact_get_too_many_keys(self):
        ...
        keys = [
            {"Key": {"pk": f"user#{i}", "sk": f"user#{i}"}, "TableName": self.table_name}
            for i in range(101)
        ]
...
class TestTransactionAtomicity(unittest.TestCase):
    ...
    def test_transaction_all_or_nothing(self):
        ...
        log = self.db.get(key={"pk": "log#001", "sk": "log#001"}, table_name=self.table_name)
        self.assertNotIn("Item", log)  # Log was NOT created

        # Account balance should be unchanged
        account = self.db.get(
            key={"pk": "account#999", "sk": "account#999"}, table_name=self.table_name
        )
